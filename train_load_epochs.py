...
if __name__ == '__main__':
    ...
    NUM_WORKERS = 4
    COLOR_MODE = 'L'
    G_CHECKPOINT_DIR = 'checkpoints'
    D_CHECKPOINT_DIR = 'checkpoints'
    os.makedirs(G_CHECKPOINT_DIR, exist_ok=True)
    os.makedirs(D_CHECKPOINT_DIR, exist_ok=True)
    ...
    # 尝试加载已有的检查点
    start_epoch = 10
    if os.path.exists(f'{G_CHECKPOINT_DIR}/netG_epoch_10.pth'):
        netG.load_state_dict(torch.load(f'{G_CHECKPOINT_DIR}/netG_epoch_10.pth'))
        print('Loaded Generator from checkpoint')
    if os.path.exists(f'{D_CHECKPOINT_DIR}/netD_epoch_10.pth'):
        netD.load_state_dict(torch.load(f'{D_CHECKPOINT_DIR}/netD_epoch_10.pth'))
        print('Loaded Discriminator from checkpoint')
    if os.path.exists(f'{G_CHECKPOINT_DIR}/training_state.pth'):
        checkpoint = torch.load(f'{G_CHECKPOINT_DIR}/training_state.pth')
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        # 保存模型参数
        torch.save(netG.state_dict(), f'{G_CHECKPOINT_DIR}/netG_epoch_{epoch}.pth')
        torch.save(netD.state_dict(), f'{D_CHECKPOINT_DIR}/netD_epoch_{epoch}.pth')
        torch.save(netG.state_dict(), f'{G_CHECKPOINT_DIR}/netG_latest.pth')
        torch.save(netD.state_dict(), f'{D_CHECKPOINT_DIR}/netD_latest.pth')
        torch.save({
            'epoch': epoch,
            'optimizerG': optimizerG.state_dict(),
            'optimizerD': optimizerD.state_dict()
        }, f'{G_CHECKPOINT_DIR}/training_state.pth')
